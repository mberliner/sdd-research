...
- **3.6-bis** **Definición operacional de R5 — [enmienda 2026-07-28, post-hoc].** *Revisión decidida al extraer R5, tras descubrir que la premisa de enforcement de PREREG §5.1 es falsa tal como se ejecutó (ver reserva mayor en §4.4). NO reescribe PREREG (sellado en `b7/prereg-v1`); lo **operacionaliza**. Implementado en `logs/r5_extract.py`.*
  ...
  - **Custodia del insumo — [nota 2026-08-01, no altera la medición].** El material del que sale R5 quedó **sin commitear** por diseño (§2.4), de modo que **no está en ningún tag `b7/*`** y no se reconstruye desde ellos como el resto de los workspaces. Al depurar los clones se selló como parches por celda en el repo de datos (`logs/R5-MATERIAL/`, uno por celda, aplicables sobre `b7/run-<x>-out` y verificados). Quien re-verifique R5 hoy MUST partir de ahí: los `ws/run-x` ya no existen en disco.
