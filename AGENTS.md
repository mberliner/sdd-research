...
1. Corré el backstop `tools/check_docs.py` con el intérprete de Python del entorno (`python`, `python3` o `py -3` según la plataforma; requiere 3.8+) y dejá el resultado en verde (0 ERROR). Cubre la parte mecánica de los checks de arriba: cobertura de spec, links markdown y rutas escritas en backticks, `[Rxx]`, campos de spec, alcance en un solo lugar y cadena de precedencia. **Verifica presencia y forma, no adecuación** — que un doc tenga spec no dice que la spec lo describa bien; eso sigue siendo tuyo. Dos checks emiten WARN como señal para que la mires, no como veredicto: `ssot-collision` (dos specs que se declaran dueñas del mismo tema) y `normative-block` (la definición de un bloque normativo reproducida fuera de su SSOT). Un WARN de esos MUST revisarse antes de entregar: o se corrige la duplicación, o se explica por qué no lo es.
