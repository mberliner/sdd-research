...
Cierre (2026-07-31): el corpus observacional se **midio y cerro**. **La pregunta de cobertura (H1) queda CERRADA como NO CONCLUYENTE**, y el motivo es un hallazgo, no una falta de datos: la **direccion de `H1` depende de la convencion de conteo** —la anatomia propia de cada formato la da a favor del hibrido, la unidad neutral en contra, y los dos extractores coinciden dentro de cada variante, luego no es ruido de modelo— y ademas el **piso de ruido del instrumento resulto de 5 a 8 veces la brecha** entre brazos. La pregunta de **fronteras (H3)** cierra como **no consistente con la hipotesis**: el brazo hibrido muestra mas colision de archivos, mas capas cruzadas y mas fan-out, robusto al *leave-one-out*, aunque con instrumento sin validacion y un modo de fallo comun no descartado. **Hallazgo transferible a toda la agenda:** un estudio que compare cobertura entre formatos de spec MUST medir su piso de ruido antes de reportar una brecha. Decision de B-07: **AJUSTAR**. Detalle: `../experimentos/b07-formato-hibrido/RESULTADO-EXPERIMENTO-B7.md` §Resultado del criterio (b).

Verificacion de instrumento (2026-08-01): el cierre de arriba se apoya en que **los dos extractores coinciden dentro de cada variante, luego no es ruido de modelo**. Esa coincidencia se habia medido con dos modelos de la **misma familia**, asi que por si sola no separaba «la regla es clara» de «la familia comparte el mismo sesgo». El corpus se replico bajo otro harness y otra familia, con pre-registro sellado antes de correr y los mismos bytes de entrada, en 13 de las 14 celdas: **bajo la regla que fija la unidad por identificador declarado las dos familias producen inventarios de identico tamaño celda por celda, y bajo la regla que deja el corte al modelo divergen**. La afirmacion de arriba queda respaldada desde una familia independiente. **No cambia ningun veredicto** y su techo es estrecho: `n`=1 por celda, MUST NOT atribuirse al harness ni al modelo por separado, y no sostiene por si sola la clausula de reproducibilidad de (b). Detalle: `../experimentos/b07-formato-hibrido/RESULTADO-EXPERIMENTO-B7.md` §Adenda 2026-08-01.

Base: GitHub Spec Kit [R10], lenguaje normativo [R04], "Power Inversion" (C3 de `ANALISIS-SPEC-KIT.md`). Experimento: `../experimentos/b07-formato-hibrido/EXPERIMENTO-B7-formato-hibrido.md`.
