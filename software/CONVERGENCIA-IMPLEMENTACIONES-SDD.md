...
**Lo que se perdió es un argumento, no una conclusión.** Hasta hoy la reserva decía «mientras Kiro no se lea». Kiro se leyó, y lo que quedó no es una respuesta sino una pregunta mejor planteada: **la defensa por fecha no está disponible en la dirección Kiro → Spec Kit / OpenSpec, para ninguno de los dos.** Establecer o descartar esa derivación exige marcadores de difusión, no cronología. Toda afirmación de invariancia de este documento MUST leerse con esa reserva, que ahora alcanza a dos columnas y no a una.

**Primera búsqueda de marcadores, sobre los clones (2026-09-06).** El trabajo dejó de estar sin hacer. Se buscó la aparición de Kiro en el historial completo de los dos repositorios vendorizados, que es el único marcador que este corpus puede verificar de forma independiente. Lo hallado:

- **Spec Kit** (primer commit 2025-08-21): la primera mención de Kiro aparece el **2026-03-03**, seis meses y medio después, y es `kiro-cli` como **integración soportada** —Kiro entra al proyecto en calidad de harness al que se le generan comandos, junto a los otros asistentes—. No hay rastro textual de Kiro en todo su período formativo, y cuando aparece no está en el rol de fuente de diseño sino en el de destino de adaptador.
- **OpenSpec** (primer commit 2025-08-05): la primera mención es del **2025-09-07**, día 33, en tres commits de README cuyo propósito declarado es posicionarse en contra (`docs(readme): streamline content, focus on change management vs Kiro`). El texto describe la disposición de artefactos de Kiro para **contrastarla**, no para adoptarla, y también soporta a Kiro como adaptador.

**Qué prueba y qué no.** Prueba que, de los tres marcadores conocidos —citar, forkear, derivar—, **ninguno aparece en la dirección Kiro → Spec Kit / OpenSpec**: lo que hay es soporte como harness en un caso y diferenciación comercial en el otro, los dos posteriores al arranque. Y no prueba independencia: la ausencia de la palabra no es ausencia de influencia, un diseño puede absorberse sin nombrarlo, y el trabajo previo al primer commit de cada repositorio no está en ningún historial —en Spec Kit ese hueco cubre justo los 38 días que siguen al anuncio de Kiro—. Es una búsqueda de marcador **negativa**, y lo más fuerte que una búsqueda así produce es «no se halló», nunca «no ocurrió». La reserva se **debilita** para las dos columnas y no se levanta.
