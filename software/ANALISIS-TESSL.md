...
[SDD-Check]
- Spec leida: SI, y **registrada antes de escribir** (`../SPECS_REGISTRY.md` -> `software/ANALISIS-TESSL.md`)
...
- Derivados a revisar: **`software/CONVERGENCIA-IMPLEMENTACIONES-SDD.md` requiere propagacion** — fila de procedencia, cierre de la deuda «Kiro y Tessl», y la dimension de C2 registrada fuera del instrumento. **`software/ANALISIS-SPEC-KIT.md` C3 queda señalado** por la precision de C1. `software/ORIENTACION-PRACTICA-IMPLEMENTACIONES-SDD.md`: Tessl entra a la poblacion con una salvedad mas dura que la de Kiro, porque el Framework esta en **beta cerrada** y por lo tanto no es adoptable hoy
