...
Las quince dimensiones convergen en un eje discriminante único, y es el que ordena la tabla: **qué relación hay entre la spec y el código, y qué la sostiene**.

**Corrección del instrumento (2026-09-07), y hay que leerla antes de la tabla.** La primera versión de esta sección tenía tres columnas —«qué es la spec», «dónde vive la autoridad», «dónde vive el enforcement»— que **no respondían la misma pregunta para los seis casos**. La primera contestaba en una fila *cuándo* (Spec Kit, «una etapa»), en otra *de qué habla* (OpenSpec, «el estado vigente»), en otra *para qué sirve* (sdd-first, «un permiso») y en otra *de quién es* (Kiro, «un artefacto del entorno»): cuatro predicados distintos en un solo encabezado, que describe pero no discrimina. La segunda usaba «autoridad» en tres sentidos incompatibles —quién gana ante discrepancia, quién define el método, quién autoriza avanzar— y en dos filas repetía con otras palabras lo que ya decía la primera o la tercera; en `sdd-first`, «en el gate» y «código fail-closed» eran la misma frase escrita dos veces. La tabla de abajo fija un predicado por columna y **«autoridad» queda definida en un solo sentido: quién gana cuando la spec y el código dicen cosas distintas**, que es el único de los tres sentidos que es propio de SDD y no de cualquier metodología.

| | Rol de la spec en la cadena spec → código | Quién gana ante discrepancia entre spec y código | Qué verifica el enforcement | Qué se paga |
|---|---|---|---|---|
| **Spec Kit** | **Entrada** del ciclo: lo produce y queda atrás salvo que el equipo elija un modelo de persistencia | **Indefinido por diseño.** La fuente no impone ninguno de los tres modelos, y no elegir es elegir flow-back, cuyo riesgo declarado es «silent divergence» | **Nada mecánico.** Comandos que el asistente ejecuta en orden; si no los ejecuta, nada falla | El costo fijo del ciclo, que no baja cuando el trabajo es chico |
| **OpenSpec** | **Registro del estado vigente**, contra el que se asientan deltas | La spec vigente, pero **en diferido**: recién manda cuando el delta se fusiona al archivar | **Nada mecánico.** El CLI sirve el método bajo demanda; si el asistente no lo invoca, no hay método y nada avisa | La fusión del delta al archivar |
| **Superpowers** | **Ninguno**: el documento de diseño es fechado y efímero, y nada se deriva de él | **No aplica**: sin spec durable no hay discrepancia posible | **Nada mecánico.** Depende de que la skill se dispare — y sus propios evals midieron la caída de 8/10 a 5/10 al quitar prosa | Contexto y tokens |
| **sdd-first** | **Precondición** de la edición: sin spec vigente declarada, no hay commit | La spec, pero **sólo sobre el permiso**: nada compara su contenido con el código que habilita | **La forma del proceso.** Código fail-closed que comprueba que la spec exista y esté vigente, no que describa lo que se escribió | Que el verificador reporte OK sin verificar |
| **Kiro** | **Artefacto sincronizado** en las dos direcciones, spec → código y código → spec | **Indefinido por diseño**, y por partida doble: la sincronización bidireccional es *flow-back* en los términos de [R10], con su «silent divergence» | **La forma del proceso.** Compuertas de fase, salteables con `Quick Spec`, más hooks por evento | Dependencia de proveedor: precio, cupos, y ningún fork posible |
| **Tessl** | **Fuente**: el código es salida, marcada `DO NOT EDIT` | **La spec, y está materializado**: el código no se edita a mano, así que la discrepancia no se resuelve — se previene | **La correspondencia.** Cada capacidad del `.spec.md` lleva su test enlazado: lo verificado es que el código haga lo que la spec dice | No-determinismo al regenerar |

### La escala de vinculación `[derivado]`

La tercera columna no es una lista de mecanismos equivalentes: son **tres niveles**, y la diferencia entre ellos es qué clase de afirmación queda respaldada.

| Nivel | Qué queda verificado | Casos |
|---|---|---|
| **N0 — nada mecánico** | Nada. El enforcement *es* la obediencia del agente: leyó la instrucción y la siguió, o no | Spec Kit, OpenSpec, Superpowers |
| **N1 — la forma del proceso** | Que hubo spec, o que se pasaron las fases. Nada mira si la spec y el código dicen lo mismo | sdd-first, Kiro |
| **N2 — la correspondencia** | Que el código cumple lo que la spec enuncia, capacidad por capacidad | Tessl |

**Cinco de los seis casos no verifican la correspondencia entre la spec y el código, y tres no verifican nada mecánicamente**, y eso se escribe sin atenuantes porque es consecuencia directa de mecanismos ya leídos en las fichas de arriba: la propia ficha de OpenSpec dice «si el asistente no invoca el CLI, no hay método. Nada avisa»; la de Superpowers, que «todo depende de que la skill se dispare»; la de Spec Kit, que la fuente «no impone ninguno» de los tres modelos de persistencia. El único caso en N2 es, además, el que está en beta cerrada, sin repositorio público, y cuyo rasgo distintivo está verificado por un tercero [R20] y no por la fuente.

**Qué es y qué no es esta escala.** Es un orden **de grado de vinculación de la spec sobre el código**, y nada más. **No** ordena por calidad, por idoneidad ni por madurez: N0 no significa peor. Significa que el método se sostiene en que alguien lo siga, y que quien adopte un caso de N0 está adoptando una convención, no una garantía — que es información para decidir, no un veredicto. Si B-09 mostrara que un caso de N0 produce resultados equivalentes a uno de N2, la escala seguiría siendo cierta como descripción y perdería relevancia práctica: **describe qué se comprueba, nunca qué funciona**.

### Los seis paradigmas, y la prueba del nombre
...
**La prueba del nombre** `[derivado]`, agregada el **2026-09-07**: quitarle a cada nombre la palabra «spec» y ver si sigue nombrando algo real y distinto. Si sobrevive, ese paradigma **no es privativo de SDD** — describe una relación entre fases, entre roles o entre herramientas, no la relación entre una especificación y el código que gobierna.

| Paradigma | ¿Sobrevive sin la palabra «spec»? | Qué otra cosa nombra igual de bien |
|---|---|---|
| Proceso ceremonial parametrizable | **Sí** | RUP, cascada, un proceso de RFC, Scrum con *Definition of Ready* |
| Control de cambios sobre un estado declarado | **Sí** | ITIL, migraciones de base de datos, un registro de ADRs |
| Disciplina de oficio | **Sí** | TDD, *pair programming*, revisión por pares |
| Cumplimiento verificado por código | **Sí** | *linters*, *policy-as-code*, cualquier gate de pre-commit |
| Método encarnado en el producto | **Sí** | un IDE opinado; *scaffolding* de framework |
| **Compilación desde la especificación** | **No** | nada: es el único irreductiblemente SDD |

Cinco de los seis nombres sobreviven, y eso dice algo del corpus y algo de este instrumento a la vez. Del corpus: la mayoría de lo que hoy se ofrece como SDD organiza **el proceso alrededor de la spec** sin establecer una relación verificable **entre la spec y el código** — lo mismo que muestra la escala de vinculación, por un camino distinto. De este instrumento: caracterizar por paradigma admite casos que no son SDD en ningún sentido fuerte, y el documento ya lo reconocía en uno solo (Superpowers, declarado ortogonal). Ahora queda declarado en cinco.

Esa coincidencia entre la prueba del nombre y la escala de vinculación **no es evidencia independiente**: las dos leen los mismos mecanismos, y las escribió la misma persona el mismo día. Es una consistencia interna del instrumento, no una confirmación.

**Cuatro advertencias sobre esta sección** `[derivado]`:
