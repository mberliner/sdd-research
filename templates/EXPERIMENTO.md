...
**Herramienta de terceros: el sello es del día de la corrida, no del diseño.** Cuando un componente es una herramienta que se actualiza sola, su versión MUST NOT fijarse al escribir el diseño: una versión elegida con antelación llega desactualizada —o no instalable— al momento de correr, y hay herramientas que publican varias versiones en un mismo día. Se sella la versión **efectiva al abrir cada tanda**, y una diferencia entre tandas es legítima: MUST registrarse y MUST declararse junto al resultado como confundido conocido y no removido (escalón 3).

**Dentro de la tanda, el escalón 1 se alcanza con una copia privada, no con un ajuste.** Desactivar la actualización automática lo hace **en ese proceso**, no en la instalación: si la herramienta es compartida, otra sesión cualquiera de la misma máquina corre su propio actualizador y mueve el ejecutable que la corrida va a lanzar. Un ajuste así deja el componente en escalón 2 creyéndose en 1, que es peor que no tener nada. El mecanismo que sí elimina la variable es **copiar el artefacto de la herramienta a una ruta privada al abrir la tanda y correr contra esa copia por ruta absoluta**. No fija una versión de antemano: congela la que haya en el momento de correr.

Esa copia es a su vez algo sellado: MUST hashearse y MUST releerse su versión desde ella —no desde la instalación compartida— en cada rep. Y MUST comprobarse en una corrida completa, no sólo pidiéndole su número de versión: credenciales, rutas de configuración y actualizadores internos pueden depender de la ruta de instalación. Si la comprobación falla, el componente vuelve a escalón 2 y eso se registra, no se omite.
